...
@section('page_title', 'Tambah Laporan Harian')

@section('content')
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-5">
        <div>
            <h3 class="fw-bold mb-1">Tambah Laporan Harian</h3>
            <div class="text-muted">{{ $project->project_name }}</div>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('pm.daily-reports.index', ['project_id' => $project->id]) }}" class="btn btn-light">
                <i class="bi bi-arrow-left me-2"></i>Kembali
            </a>
        </div>
    </div>

    <form method="POST" action="{{ route('pm.daily-reports.store') }}">
        @csrf
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="weather_condition" id="weather_condition" class="form-select select2-header" required>
                            ...
                            @foreach(['Cerah','Hujan','Mendung'] as $w)
                                <option value="{{ $w }}" @selected(old('weather_condition') === $w)>{{ $w }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Waktu Cuaca</label>
                        <input type="text" name="weather_time" class="form-control" value="{{ old('weather_time') }}" placeholder="Contoh: Pagi / Siang / Sore">
                    </div>
                    <div class="col-12">
                        <label class="form-label">Catatan Cuaca</label>
                        <textarea name="weather_notes" class="form-control" rows="3">{{ old('weather_notes') }}</textarea>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="supervisor_id" id="supervisor_id" class="form-select select2-header">
                            ...
                            @foreach($teamEmployees as $pe)
                                <option value="{{ $pe->employee_id }}" @selected(old('supervisor_id') === $pe->employee_id)>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="executor_id" id="executor_id" class="form-select select2-header">
                            <option value="">- pilih -</option>
                            @foreach($teamEmployees as $pe)
                                <option value="{{ $pe->employee_id }}" @selected(old('executor_id') === $pe->employee_id)>
                                    {{ $pe->employee?->employee_name ?? '-' }} - {{ $pe->projectRole?->role_name ?? '-' }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <tbody id="worksBody">
                            ...
                            @else
                                <tr class="work-row">
                                    ...
                                    <td>
                                        <input type="number" step="0.01" min="0" name="works[0][volume]" class="form-control" value="0" required>
                                    </td>
                                    <td class="text-end">
                                        <button type="button" class="btn btn-sm btn-light-danger btnRemoveWork"><i class="bi bi-trash"></i></button>
                                    </td>
